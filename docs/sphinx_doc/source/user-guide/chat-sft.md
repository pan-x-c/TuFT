...
## Table of Contents
1. [When to Use SFT vs. RL](#when-to-use-sft-vs-rl)  
2. [Datasets](#datasets)  
3. [Minimal Training Example (SFT)](#minimal-training-example-sft)  
4. [Key Concepts](#key-concepts)  
   - [Chat Formatting & Templates](#chat-formatting--templates)  
   - [Loss Masking (Assistant-only)](#loss-masking-assistant-only)  
...
**TuFT** (Tenant-unified FineTuning) is a multi-tenant system that provides a unified service API for fine-tuning large language models (LLMs). It supports the Tinker service API and can be used with the Tinker SDK. Unlike the Tinker, TuFT can run on local GPUs; the experiments below were conducted on a local **2× NVIDIA A100-SXM4-80GB** setup (Driver 550.54.15, CUDA 12.9). Before running the example, follow the [Installation Guide](../getting-started/installation.md) to start the TuFT server locally.
