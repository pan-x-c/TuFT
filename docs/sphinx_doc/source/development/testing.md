...
The `conftest.py` automatically sets the environment variable `TUFT_CPU_TEST=1` when `--gpu` is not specified, which configures backends to use CPU-compatible implementations during testing. Tests marked with `@pytest.mark.gpu` are automatically skipped.
